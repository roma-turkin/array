...
Массив реализован в виде двоичного дерева поиска без функции балансировки..
...
/*
    В функции create_array выделяется память под один узел - root. В root->index лежит -1 - так мы отличаем корень дерева от всех остальных
узлов. (полагается, что это нормальный Си-шный массив с индексами от 0 и до скольки угодно). В data рута нет ничего.
*/
ARRAY create_array(){
    struct node * root;
...
/*
    Insert получает в качестве аргументов массив, в который надо вставить элемент, индекс элемента и указатель на место памяти, 
...
int insert(ARRAY array,INDEX input_index,DATA input_data){	
    ...
    leaf->left = NULL;
    if (!root->left)
    {				                //root - просто корень, он не является элементом массива. Первый элемент массива лежит слева от root.
